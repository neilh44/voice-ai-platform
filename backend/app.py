...
@app.route('/api/call/<user_id>/<call_sid>/recordings', methods=['GET'])
def get_call_recordings(user_id, call_sid):
    """API endpoint to get recordings for a specific call"""
    try:
        # Initialize TwilioService
        twilio_service = TwilioService()
        
        # Get recordings for the call
        recordings = twilio_service.get_call_recordings(user_id, call_sid)
        
        return jsonify(recordings)
        
    except Exception as e:
        logging.error(f"Error getting call recordings: {str(e)}")
        return jsonify({"error": str(e)}), 500

@app.route('/api/call/<call_sid>/transcript', methods=['GET'])
def get_call_transcript(call_sid):
    """API endpoint to get the transcript for a specific call"""
    try:
        # Query the database for transcriptions related to this call
        conn = sqlite3.connect('voiceai.db')
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        ...
        # If no transcriptions found in our database, try Twilio API as backup
        if not transcript_parts:
            # Since we don't know which user this call belongs to, we'll need to query
            # user_calls table to find the user_id for this call
            conn = sqlite3.connect('voiceai.db')
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            
            cursor.execute('SELECT user_id FROM user_calls WHERE call_sid = ?', (call_sid,))
            result = cursor.fetchone()
            conn.close()
            
            if result:
                user_id = result['user_id']
                # Get the transcription from Twilio API
                twilio_service = TwilioService()
                recordings = twilio_service.get_call_recordings(user_id, call_sid)
        ...
        return jsonify({
            ...
            'full_transcript': ' '.join([t['text'] for t in transcript_parts if t['text']])
        })
        
    except Exception as e:
        logging.error(f"Error getting call transcript: {str(e)}")
        return jsonify({"error": str(e)}), 500
